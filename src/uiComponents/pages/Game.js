...
export default (props) => {
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === 'development')
      dispatch(
        loadLevelData(worldAndLevelIDs.TEST_WORLD, worldAndLevelIDs.LEVEL_1),
      )
    else
      dispatch(
        loadLevelData(worldAndLevelIDs.WORLD_1, worldAndLevelIDs.LEVEL_1),
      )
  ...
  return (
    <>
      ...
      <CanvasPageContainer
        style={{ display: `${loadingLevel === true ? 'none' : 'block'}` }}
      >
        <GameCanvas />
      </CanvasPageContainer>
    </>
